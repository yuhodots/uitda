...
module.exports = {
    ...
    hasPost: function (request, board) { // 로그인 유저가 게시글을 하나라도 가지고 있는지 판단
        db.query(
    ...
    sameOwner_carpool: function (request, username) { // 로그인 유저와 글 저자가 같은지 판단
    ...
    hasPost_carpool: function (request, board) { // 로그인 유저가 게시글을 하나라도 가지고 있는지 판단
        db.query(
            `SELECT EXISTS(SELECT * FROM ${board} WHERE username='${request.user.username}') AS SUCCESS`,
            function (error, result) {
                if (error) throw error;
                else {
                    console.log(result);
